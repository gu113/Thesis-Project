...
# Import Agents
from agents.ddqn_agent import DDQNAgent, DDQNAgentPER

# Import Models
from models.ddqn_cnn import DDQNCnn
...
GAMMA = 0.99
BUFFER_SIZE = 50000                # Average Memory Usage: 10k -> 2GB       50k -> 5GB      100k -> 8GB     200k -> 12GB
BATCH_SIZE = 64                     # Jetson Recommended Values: 32 or 16
LR = 0.00025
TAU = 0.001
UPDATE_EVERY = 4
EPS_START = 0.99
EPS_END = 0.01
EPS_DECAY = 500                    # Jetson Recommended Values: Long Term Training: 10000

# Initialize Agents
agent = DDQNAgentPER(INPUT_SHAPE, ACTION_SIZE, 0, device, BUFFER_SIZE, BATCH_SIZE, GAMMA, LR, TAU, UPDATE_EVERY, 10000, DDQNCnn)
load_agent(agent, device, 'trained_models/Pong_DDQN_Embedded.pth')
...
save_agent(agent, 'trained_models/Pong_DDQN_Embedded.pth')     #Target Score: Agent 21 x 0 Opponent
