...
GAMMA = 0.99
BUFFER_SIZE = 100000 #10k - +-2GB     50k-5GB   100k - +-8GB   Jetson Recommended 10k or 20k
BATCH_SIZE = 64 #Jetson Recommended 32 or 16
LR = 0.0001
TAU = 0.001
UPDATE_EVERY = 8 #4 or 8
EPS_START = 0.50
EPS_END = 0.01
EPS_DECAY = 1000
...
#load_agent(agent, device, 'trained_models/Pong_strict_test.pth')
...
def train(n_episodes):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        # Print Progress
        if i_episode % 10 == 0:
            print(f"Episode {i_episode:5d} | Avg Agent Score: {np.mean(agent_score_window):.2f} x Avg Opponent Score: {np.mean(opponent_score_window)} | Epsilon: {eps:.2f}")
...
save_agent(agent, 'trained_models/Pong_strict_test.pth')     #Jetson-1k                        # Trained for:2k Episodes           Target Score: Agent 21 x 0 Opponent
save_features(agent, 'trained_models/Pong_strict_test_features.pth')
